...
function seedPast() {
  const base = [148, 172, 96, 205, 133, 161];
  const igs  = [58, 63, 44, 68, 51, 57];
  for (let i = 6; i >= 1; i--) {
    const d = new Date(); d.setDate(d.getDate() - i);
    PAST.push({ d, carbs: base[6 - i], ig: igs[6 - i], meals: 3 + (i % 2) });
  }
}
...
$$('#gpFeel [data-feel]').forEach(b => b.onclick = () => {
  ...
  gpLog.unshift({ t: new Date(), f, meal: last ? last.name : 'hors repas' });
  if (gpLog.length > 12) gpLog.pop();
  renderGpLog();
...
function saveState() {
  store.set('glycia.data', JSON.stringify({
    ...
    favorites: FAVORITES,
    gp: state.gp,
    past: PAST.map(p => ({ d: p.d.toISOString(), carbs: p.carbs, ig: p.ig, meals: p.meals }))
  }));
}
...
function loadState() {
  ...
  try {
    ...
    if (o.day === dayKey(new Date())) {
      ...
      PAST.push({ d: new Date(o.day + 'T12:00:00'), carbs: c, ig: c ? Math.round(num / c) : 0, meals: o.journal.length });
