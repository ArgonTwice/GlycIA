...
$('#foodQ').addEventListener('input', e => {
  ...
  if (fq.length >= 3 && matchFoods().length < 6) offTimer = setTimeout(async () => {
    if (await ensureCiqual()) {
      renderFoods(`Table Ciqual ouverte — ${ALL.length.toLocaleString('fr-FR')} aliments`);
      if (matchFoods().length >= 6) return;
    }
    if (await ensureOffFr()) {
      renderFoods(`Produits français ouverts — ${ALL.length.toLocaleString('fr-FR')} aliments hors ligne`);
      if (matchFoods().length >= 6) return;
    }
    await offSearch(fq);
...
function ensureOffFr() {
  ...
  offFrEnCours = (async () => {
    try {
      ...
      const d = await r.json();
      for (const [n, c, kcal, lip, code] of d.aliments) {
        registerFood({ n, cat: OFFFR_CAT, c, ig: guessIG(n, c), kcal,
...
if (globalThis.__GLYCIA_TEST__) Object.assign(globalThis.__GLYCIA_TEST__, {
  normalize, clamp, round, esc, fmtQ, igClass, igLabel, guessIG, withBrand,
