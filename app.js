...
/* L'inverse d'esc(), pour ce qui ARRIVE. Des noms de produits d'Open Food
   Facts et de l'USDA portent des entités HTML : esc() rééchappe alors le &,
   et l'utilisateur lit « Quinoa cuit à l&apos;eau » au lieu de l'apostrophe.
   Les bases livrées avec l'app sont nettoyées à la génération ; ceci couvre
   ce qui vient du réseau, hors de notre contrôle.
   Certaines saisies sont encodées deux fois — « Pik&amp;amp;croq » veut dire
   « Pik&croq » — d'où la reprise, bornée : sans borne, un nom contenant
   vraiment « &amp; » comme texte se ferait déshabiller indéfiniment. */
const ENTITES = { apos: '\'', quot: '"', lt: '<', gt: '>', nbsp: ' ', amp: '&' };
function unent(s) {
  let v = String(s);
  for (let i = 0; i < 3; i++) {
    const w = v.replace(/&#(\d+);/g, (_, n) => String.fromCodePoint(+n))
...
const HYPO_FIN = 'glycia.hypoFin';       // instant de fin, pour survivre à la veille

/* Le minuteur se cale sur l'horloge, pas sur ses propres battements.
   Un `setInterval` qui décrémente un compteur suppose que chaque seconde
   arrive : sur un téléphone, l'onglet en arrière-plan est ralenti et l'écran
   verrouillé le fige tout à fait. Quinze minutes de pause devenaient vingt ou
   trente sans que rien ne le signale — et sur cet écran-là, se tromper sur le
   moment de recontrôler n'est pas anodin.

   Corollaire : la fin est un instant, pas un décompte. L'app peut être fermée
   et rouverte, le minuteur reprend où il en est. */
let tFin = null;                          // instant de fin, ms epoch, ou null
let tLeft = T_TOTAL, tId = null, tRunning = false;

const restant = () => tFin === null ? tLeft : Math.max(0, Math.round((tFin - Date.now()) / 1000));
...
/* Garder l'écran allumé pendant la pause : le poser et le voir s'éteindre,
   c'est manquer la fin du minuteur. Silencieux si le navigateur ne sait pas. */
let veille = null;
async function ecranAllume(on) {
  try {
    if (on) {
      if (!veille && 'wakeLock' in navigator) veille = await navigator.wakeLock.request('screen');
...
/* Les paliers se déclenchent au FRANCHISSEMENT, pas sur une valeur exacte :
   une seconde sautée — et il en saute, dès que l'onglet passe derrière — et
   l'annonce ne serait jamais venue.

   Le texte des 5 minutes disait « 5 minutes passées » à un moment où il en
   restait 5, donc où 10 étaient passées. Sur cet écran, se tromper de sens
   n'est pas une coquille. */
function tick() {
  const avant = tLeft;
  tLeft = restant();
  paintTimer();
  if (avant > 600 && tLeft <= 600 && hypoMode) speakFr('Encore dix minutes.');
  if (avant > 300 && tLeft <= 300) {
    $('#timerMsg').textContent = '10 minutes passées, encore 5. Reste assis, ça remonte.';
...
function stopTimer() {
  clearInterval(tId); tId = null; tRunning = false;
  tLeft = restant(); tFin = null;
  store.del(HYPO_FIN);
  ecranAllume(false);
  timerBtn(false);
}
function startTimer() {
  if (tRunning) return;
  if (tLeft <= 0) tLeft = T_TOTAL;
  tFin = Date.now() + tLeft * 1000;
  store.set(HYPO_FIN, String(tFin));
...
/* Au retour d'un écran verrouillé, on recale tout de suite plutôt que
   d'attendre le prochain battement — et on redemande l'écran allumé, que le
   système a repris pendant l'absence. */
document.addEventListener('visibilitychange', () => {
  if (document.hidden || !tRunning) return;
  tick();
  if (tRunning) ecranAllume(true);
});

/* Le minuteur survit à la fermeture de l'app : c'est un instant de fin qui est
   gardé, pas un décompte. Rouvrir GlycIA au milieu d'une hypo doit retrouver
   la pause en cours, pas la remettre à quinze minutes. */
function reprendreTimer() {
  const fin = +store.get(HYPO_FIN);
  if (!fin || !isFinite(fin)) return;
  if (fin <= Date.now()) { store.del(HYPO_FIN); return; }
  tFin = fin;
  tLeft = restant();
  tRunning = true; tId = setInterval(tick, 1000); timerBtn(true);
  paintTimer();
  ecranAllume(true);
  $('#timerMsg').textContent = 'Pause reprise là où tu l’avais laissée.';
}

function chime() {
  try {
    const ac = new (window.AudioContext || window.webkitAudioContext)();
    /* Un contexte audio créé hors geste direct démarre suspendu : sans ça, le
       signal de fin ne sonnait pas — le seul moment où il sert vraiment. */
...
$('#timerReset').addEventListener('click', () => {
  stopTimer(); tFin = null; tLeft = T_TOTAL; paintTimer();
  $('#timerMsg').classList.remove('done');
    ...
    <div class="item">
      ...
      <div class="item-body">
        <div class="item-name">${esc(f.n)}${(() => {
          ...
          return gpLevel(probe) === 3 ? `<span class="gp-badge no" title="${esc(gpReason(probe).map(r => r.t).join(' '))}">⛔</span>` : '';
        })()}</div>
...
/* Valeur d'origine des aliments dont la table a corrigé l'IG. Elle ne sert
   plus à l'affichage, mais la table d'IG personnelle en a besoin : voir
   migrateIgPerso(). */
const IG_ORIG = new Map();
...
Object.entries(FOODDB).forEach(([cat, list]) => list.forEach(a => {
  ...
  if (tr && a[2] != null && a[2] !== tr.ig) IG_ORIG.set(key, a[2]);
...
function offToEntry(p) {
  ...
  let name = unent(p.product_name_fr || p.product_name || '').trim();
  if (!name) return null;
...
function usdaToEntry(p) {
  ...
  const nom = unent(p.description || '').trim();
  if (!nom) return null;
...
/* Longueur du nom seul, deduite de la cle de recherche sans renormaliser :
   f.k vaut normalize(nom + ' ' + categorie), et il n'y a qu'une vingtaine de
   categories, donc leur longueur normalisee se retient. */
const CAT_LEN = new Map();
function nomLen(f) {
  let v = CAT_LEN.get(f.cat);
  if (v === undefined) { v = normalize(f.cat).length; CAT_LEN.set(f.cat, v); }
  return f.k.length - v - 1;
}

/* « Riz au lait » n'est pas du riz, « Pomme de terre » n'est pas une pomme.
   En francais, une preposition juste apres le mot cherche annonce un autre
   aliment, pas une precision sur celui-ci. Ces aliments restent affiches,
   simplement apres ceux que la recherche visait vraiment. */
const PREP = /^(?:aux?|a l[ae]?|del?|du|des|en|sauce|facon|nature)\b/;

/* 0 le nom EST la requete · 1 le nom commence par elle · 2 il commence par
   elle suivie d'une preposition · 3 elle est ailleurs, dans un autre mot du
   nom ou dans la categorie. */
function rangRecherche(f) {
  if (!f.k.startsWith(fq)) return 3;
  const nl = nomLen(f);
  if (nl === fq.length) return 0;
  /* Le « s » du pluriel est tolere : « pommes de terre » repond a « pomme ».
     Le « a » seul n'entre pas dans PREP, lui — « pain a burger » reste un
     pain, la ou « riz au lait » n'est plus du riz. */
  const suite = /^s? (.*)$/.exec(f.k.slice(fq.length));
  return suite && PREP.test(suite[1]) ? 2 : 1;
}
...
function matchFoods() {
  ...
  if (fq) {
    /* Le rang se calcule une fois par aliment, pas a chaque comparaison :
       une version passee appelait normalize() deux fois par comparaison, soit
       des centaines de milliers d'appels par frappe. */
    for (const f of out) f._r = rangRecherche(f);
    /* A pertinence egale, le noyau passe devant les bases etendues : ses
       aliments sont choisis et portionnes, les autres sont la longue traine.
       Sans ca, chercher « riz » remontait quatre « riz au lait » de Ciqual
       avant le riz lui-meme. */
    out.sort((a, b) => a._r - b._r
      || (CATS_ETENDUES.has(a.cat) ? 1 : 0) - (CATS_ETENDUES.has(b.cat) ? 1 : 0)
      || parNom(a, b));
  }
  ...
  $('#foodList').innerHTML = shown.map(f => {
    ...
    <div class="frow ${fOpen === f.n ? 'open' : ''}">
      <button class="fhead" data-food="${esc(f.n)}">
        ...
        <span class="fn"><b>${esc(f.n)}${state.gp ? `<span class="gp-badge ${GP_MARK[gpLevel(f)][0]}" title="${esc(gpReason(f).map(r => r.t).join(' '))}">${GP_MARK[gpLevel(f)][1]} ${GP_MARK[gpLevel(f)][2]}</span>` : ''}</b><span>${esc(f.pl)} · ${f.pw} g · ${igLabel(f.ig)}</span></span>
...
/* ---------- Reprise des observations d'avant les IG tracés ----------
   Une observation garde un rapport : écart mesuré ÷ charge glycémique
   attendue. Cette charge dépend de l'IG en vigueur au moment du repas. Quand
   la table tracée a corrigé l'IG d'un aliment — quinoa 35 → 53 — les rapports
   déjà enregistrés se retrouvent sur une autre échelle que celle que
   personalIg() suppose, et « IG chez toi » se met à mentir dans le sens de
   la correction.

   Rien n'est perdu : l'ancienne valeur est toujours dans db.json, la table ne
   fait que la recouvrir. Le rapport se ramène donc exactement sur la nouvelle
   base, sans jeter une seule observation. */
const IGP_VER = 'glycia.igpersoVer';
function migrateIgPerso() {
  if (store.get(IGP_VER) === '2') return;
  let corriges = 0;
  IG_ORIG.forEach((ig0, key) => {
    const arr = IGP.obs[key], e = IG_TRACE.get(key);
    if (!arr || !arr.length || !e || !e.ig) return;
    IGP.obs[key] = arr.map(v => round(v * ig0 / e.ig, 3));
    corriges++;
  });
  store.set(IGP_VER, '2');
  if (corriges) saveIgPerso();
  return corriges;
}
...
/* ==========================================================================
   ...
   ========================================================================== */
/* ---------- Clés de correspondance dérivées de GP_FOODS ----------
   GP_FOODS est une table de conseils, écrite pour être lue : « Chou, brocoli,
   chou-fleur », « Fruits secs (abricots, pruneaux) », « Tomate pelée et
   épépinée ». S'en servir telle quelle pour apparier des aliments confond
   deux formes que la lecture, elle, distingue sans effort :

   - une énumération d'aliments de même rang — « Chou, brocoli, chou-fleur » —
     dont chaque terme vaut pour lui-même ;
   - une préparation — « Tomate pelée et épépinée » — dont le qualificatif est
     tout le propos : une tomate crue n'est pas une tomate pelée et épépinée.

   Les exemples entre parenthèses sont déjà couverts par leur tête de liste
   (« Fruits secs ») et ne deviennent donc pas des clés. Sans cette règle,
   « abricots » en devenait une, et un abricot frais héritait du niveau des
   fruits secs — il s'affichait « ⛔ à éviter », au motif d'être un « plat
   composé ». 45 aliments étaient classés de cette façon. */
const GP_KEYS = [];
Object.values(GP_FOODS).forEach(l => l.forEach(([n, lv]) => {
  normalize(n).replace(/\([^)]*\)/g, ' ').split(',')
    .map(s => s.trim()).filter(Boolean)
    /* Le libellé d'origine est gardé : c'est lui qu'on citera comme motif,
       plutôt que de laisser la raison se chercher ailleurs et dire autre
       chose que le classement. */
    .forEach(t => GP_KEYS.push([t, lv, n]));
}));
/* La clé la plus longue gagne : « riz complet » doit primer sur « riz ». */
GP_KEYS.sort((a, b) => b[0].length - a[0].length);

/* Correspondance sur des mots entiers, pluriel toléré : sans ça « ail » se
   reconnaît dans « ailerons de poulet ». */
function gpMot(nom, cle) {
  let i = nom.indexOf(cle);
  while (i >= 0) {
    const avantOk = i === 0 || !/[a-z0-9]/.test(nom[i - 1]);
    let j = i + cle.length;
    if (nom[j] === 's') j++;
    if (avantOk && (j === nom.length || !/[a-z0-9]/.test(nom[j]))) return true;
    i = nom.indexOf(cle, i + 1);
  }
  return false;
}

/* Chaque motif désigne un mécanisme de GP_MECA, qui porte un titre court et
   l'explication du frein. GP_RED et GP_GREEN sont dérivées de ces tables : le
   classement affiché et la raison donnée ne peuvent pas diverger.
...
const GP_RED_WHY = [
  /* « baguette » est sortie d'ici : une baguette blanche n'a ni son ni
     céréales complètes, et la table de repères la classe déjà pour ce
     qu'elle est — du pain frais. */
  [/complet|cereale|granola|muesli|\bmais\b|pop-?corn|seigle|son d/,
   'fibres_dures'],
  [/graine|noix|amande|pistache|cajou|noisette|pecan|macadamia|cacahuete/,
   'lipides'],
  /* « sec » ne vaut ici que pour les légumes secs : le laisser seul classait
     « Vin blanc sec » et « Chèvre sec » parmi les légumes fibreux. */
  [/seche|legumes? secs?|cru$|crudite|chou|brocoli|poireau|celeri|fenouil|salade|artichaut|asperge/,
   'legume_fibreux'],
  /* Le haricot vert se mange en gousse, pas en graine : ce n'est pas une
     légumineuse, et l'explication ci-dessous ne lui convient pas. */
  [/lentille|haricot(?!s? (?:vert|plat))|feve|legumineuse|pois chiche|petits pois|flageolet/,
   'legumineuses'],
...
/* [message propre à la catégorie, mécanisme qui l'explique]. Sans le second
   champ, ces aliments — les 200 plats de chaîne et de terroir — recevaient un
   message de catégorie et rien de plus : ils étaient les seuls à ne pas
   pouvoir dire quel frein leur était reproché. */
const GP_CAT_WHY = {
  'Fast-food & sandwichs': ['Plat de restauration rapide : gras et dense, deux facteurs qui ralentissent la vidange.', 'lipides'],
  'Sauces, matières grasses & apéro': ['Surtout des matières grasses, le premier frein à la vidange de l’estomac.', 'lipides'],
  'Cuisine du monde': ['Plat composé, souvent gras ou relevé, difficile à anticiper avec un estomac lent.', 'lipides'],
  'Spécialités régionales françaises': ['Plat de terroir, généralement riche en gras et long à quitter l’estomac.', 'lipides']
};
...
/* Le motif court reste visible, l'explication du mécanisme se déplie. Un
   classement qu'on ne peut pas contester n'est qu'une interdiction de plus,
   et c'est précisément ce que l'app refuse d'être : la fiche doit dire quel
...
function gpReason(f) {
  ...
  /* Le nom de l'aliment d'abord. S'il ne dit rien, on interroge le libellé du
     repère qui a décidé du niveau : « Steak, rôti, viande en morceaux » nomme
     un frein que « Rôti de bœuf » seul ne laissait pas voir. Sans ça, la fiche
     renvoyait à la table sans expliquer quoi que ce soit. */
  let reds = GP_RED_WHY.filter(([re]) => re.test(k)).slice(0, 2).map(([, id]) => meca(id));
  if (!reds.length && f._gpsrc) {
    const ks = normalize(f._gpsrc);
    reds = GP_RED_WHY.filter(([re]) => re.test(ks)).slice(0, 2).map(([, id]) => meca(id));
  }
  const green = GP_GREEN_WHY.find(([re]) => re.test(k));
  const fat = gpFat(f);
  const out = [...reds];

  /* Quand le niveau vient de la table de repères, on cite la ligne qui l'a
     décidé. Sans ça, le classement et la raison se cherchaient chacun de leur
     côté et pouvaient se contredire : un rôti de bœuf sortait « à éviter »
     au motif d'être un « plat composé », alors que le repère disait
     simplement « Steak, rôti, viande en morceaux ». */
  const repere = f._gpsrc
    ? { t: `Repère de la table gastroparésie : « ${f._gpsrc} ».`,
        p: 'Ce classement vient de la table de repères de l’app, pas d’une règle calculée sur les valeurs de l’aliment.' }
    : null;

  /* Le chiffre de lipides est propre à l'aliment : il ne peut pas venir d'une
  ...
  if (lv === 3) {
    ...
    if (!out.length) out.push({
      t: 'Plat composé, souvent gras ou fibreux, long à réduire en bouillie.',
      p: 'Aucun frein précis n’a pu être nommé pour cet aliment : le classement vient de sa catégorie, pas de ce qu’il contient. À prendre pour ce que c’est — une approximation.'
    });
    if (green) out.push(meca('mixage'));
  } else if (lv === 1) {
    out.length = 0;
    ...
    /* Un repère explicite vaut mieux qu'un motif attrapé au passage : la
       baguette blanche s'expliquait par les « céréales complètes et le son ». */
    if (repere) { out.length = 0; out.push(repere); }
    if (!out.length) out.push(meca('quantite'));
  }
  return out;
}
...
function gpRecipeReason(r) {
  if (r.ph) return [{ t: 'Recette du répertoire gastroparésie : texture et cuisson déjà adaptées.', p: '' }];
  const txt = normalize(r.title + ' ' + r.ing.map(x => x[0]).join(' '));
...
loadMyFoods();
loadIgPerso();
reprendreTimer();          // une pause hypo en cours doit se retrouver au demarrage
migrateIgPerso();        // ramène les observations d'avant sur la nouvelle base d'IG
loadProfile();
renderFavorites();
setMascot('hello');
renderDay();
...
if (globalThis.__GLYCIA_TEST__) Object.assign(globalThis.__GLYCIA_TEST__, {
  ...
  startTimer, stopTimer, reprendreTimer, restant, T_TOTAL, HYPO_FIN,
  timerEnCours: () => tRunning,
  ...
  matchFoods, setQuery: q => { fq = normalize(q); }
});
